...
class Mailer < ActionMailer::Base
  ...
  # Send result of processing of incoming message.
  def self.deliver_incoming_mail_responce(message, status)
    mail_data = Mail.new(message.b)
    user_email = mail_data.from.to_a.first.to_s.strip
    subject = mail_data.subject.to_s
    user = User.find_by_mail(user_email)
    incoming_mail_responce(user, subject, status).deliver_later
  end

  # Build mail with result of processing of received mail.
  def incoming_mail_responce(user, subject, status)
    if status
      @message = "Your message [#{subject}] successfully received."
    else
      @message = "While processing your message [#{subject}] next error occurred:"
      # Gets log message which was raised by MailHandler
      @exception = IO.readlines("./log/#{Rails.env}.log")[-100..-1]
                     .map {|line| line if line.match(/^MailHandler: [\w\s:]+$/) }.compact.last
    end
    mail :to => user,
         :subject => 'Mail status'
  end
