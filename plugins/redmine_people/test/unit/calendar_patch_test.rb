...
class CalendarPatchTest < ActiveSupport::TestCase
  ...
  def test_holyday_on_next_year
    @calendar.custom_events = PeopleHoliday.between(Date.today.end_of_year, Date.today.end_of_year + 10.days).to_a

    check_working_and_non_working_days_cases [], [
      { from: Date.today.end_of_year, to: Date.today.end_of_year + 10.days, non_working_days_list: [Date.today.end_of_year, Date.today.end_of_year + 6.days] }
    ]
  end
