...
Redmine::Plugin.register :redmine_people do
  ...
  menu :top_menu, :people, { :controller => 'people_settings', :action => 'index'}, :caption => :label_employee_management, :if => Proc.new {
    User.current.allowed_people_to?(:view_people)
  }

  menu :admin_menu, :people, { :controller => 'people_settings', :action => 'index' }, :caption => :label_employee_management, :html => { :class => 'icon' }
end
