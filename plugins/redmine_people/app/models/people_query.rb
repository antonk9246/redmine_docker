...
class PeopleQuery < Query
  ...
  def initialize_available_filters
    ...
    add_available_filter('people_tags', type: :people_tags, name: l(:label_people_tags_plural), values: [], order: 19)
  ...
  def sql_for_people_tags_field(_field, operator, value)
    person = Person.where({})
    case operator
    when '=', '!'
      person = person.tagged_with(value, match_all: true)
    when '!*'
      person = person.joins(:tags).uniq
    else
      person = person.tagged_with([RedmineCrm::Tag.all.map(&:to_s)], any: true)
    end

    compare   = operator.include?('!') ? 'NOT IN' : 'IN'
    ids_list  = person.collect(&:id).push(0).join(',')
