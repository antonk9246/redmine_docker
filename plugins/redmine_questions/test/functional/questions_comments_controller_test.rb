...
class QuestionsCommentsControllerTest < ActionController::TestCase
  ...
  def test_create_comment_for_answer
    @request.session[:user_id] = 1
    old_comment_count = questions_answers(:answer_001).comments.count
    assert_difference 'Comment.count', 1 do
      compatible_request :post, :create, :source_id => questions_answers(:answer_001),
                    :source_type => 'questions_answer',
                    :comment => {
                      :comments => 'text for comment'
                    }
    end
    assert_equal 'text for comment', questions_answers(:answer_001).comments.last.comments
  end
