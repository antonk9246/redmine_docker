...
class QuestionsControllerTest < ActionController::TestCase
  ...
  def test_get_index
    ...
    tags = Question.tags_cloud(:project => @project)
    assert_select 'ul.questions-tags li', {:count => tags.to_a.count}
  end
  ...
  def test_get_show
    ...
    assert_select 'input#answer_accepted'
    assert_select 'a.vote-up', href: /#{questions(:question_001).to_param}/
    assert_select 'span.items', {:text => "(1-2/2)"}
    @dev_role.permissions << :vote_questions
    @dev_role.save
    ...
    assert_select 'div#reply', :count => 0
    assert_select 'a.icon-del', :count => 0
    assert_select 'input#answer_accepted', :count => 0
    assert_select 'a.vote-up', href: /#{questions(:question_001).to_param}/
    ...
    assert_select 'div h1', :text => questions(:question_001).subject
    assert_match questions(:question_001).content, @response.body
    assert_select 'div.add_comments .add-comment-form textarea', :count => 3
        assert_select 'div#reply'
    assert_select 'a.icon-del'
    @dev_role.permissions << :accept_answers
    @dev_role.save
    @request.session[:user_id] = 3
    compatible_request :get, :show, :id => 1
    assert_response :success
    assert_select 'div h1', :text => questions(:question_001).subject
    assert_match questions(:question_001).content, @response.body
    assert_select 'input#answer_accepted', :count => 1
    assert_select 'a.vote-up', href: /#{questions(:question_001).to_param}/

    @dev_role.remove_permission! :edit_questions, :edit_own_answers
    @dev_role.add_permission! :accept_answers
    @dev_role.save
    @request.session[:user_id] = 3
    compatible_request :get, :show, :id => 1
    assert_select '.question.answer a.icon.icon-accept'
    assert_select '.question.answer a.icon.icon-discard'
    assert_select '.question.answer a.icon.icon-edit', :count => 0

    @dev_role.remove_permission! :edit_questions
    @dev_role.add_permission! :edit_own_answers
    @dev_role.save
    @request.session[:user_id] = 3
    compatible_request :get, :show, :id => 1
    assert_select '.question.answer a.icon.icon-edit', :count => 1
  ...
  def test_update_form
    @request.session[:user_id] = 1
    section = questions_sections(:section_005)
    compatible_xhr_request :put, :update_form, :project_id => @project,
      :question => {:section_id => section.id}
    assert_response :success
    assert_match 'text/javascript', response.content_type
    assert_match '<label>Status', response.body
  end
