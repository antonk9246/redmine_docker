...
class QuestionTest < ActiveSupport::TestCase
  ...
  def test_create_with_tag_list
    new_question = Question.new
    new_question.subject = 'Some new question'
    new_question.author = users(:users_001)
    new_question.tag_list = 'first tag'
    new_question.section = questions_sections(:section_001)
    assert !new_question.save, 'Faild save question with tag list'
    new_question.content = 'test content'
    assert new_question.save
    assert Question.tagged_with('first tag').include?(new_question)
  end

  def test_question_votable
    assert Question.votable?, 'Question must be votable'
  end

  def test_question_taggable
    assert Question.taggable?, 'Question must be taggable'
  end
