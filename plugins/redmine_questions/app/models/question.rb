...
class Question < ActiveRecord::Base
  ...
  safe_attributes 'featured', 'locked',
    :if => lambda {|question, user| question.section && user.allowed_to?(:manage_sections, question.project)}
  ...
  def self.tags_cloud(options = {})
    ...
    join << "JOIN #{Question.table_name} ON #{Question.table_name}.id = #{RedmineCrm::Tagging.table_name}.taggable_id AND #{RedmineCrm::Tagging.table_name}.taggable_type =  '#{Question.name}' "
    join << "JOIN #{QuestionsSection.table_name} ON #{QuestionsSection.table_name}.id = #{Question.table_name}.section_id"
    join << "JOIN #{Project.table_name} ON #{Project.table_name}.id = #{QuestionsSection.table_name}.project_id"
    ...
    scope = scope.where(Project.allowed_to_condition(options[:user] || User.current, :view_questions))
    scope = scope.where("#{QuestionsSection.table_name}.project_id = ?", options[:project]) if options[:project]
    scope = scope.where("#{Question.table_name}.section_id = ?", options[:section]) if options[:section]
