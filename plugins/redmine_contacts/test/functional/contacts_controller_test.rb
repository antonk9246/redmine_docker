...
class ContactsControllerTest < ActionController::TestCase
  ...
  def test_filter_by_ids_not_equal
    ...
    assert_response :success
    assert_equal [3, 4, 5, 6].sort, contacts_in_list.map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'
  ...
  def test_filter_by_ids_any
    ...
    assert_response :success
    assert_equal [1, 2, 3, 4, 5, 6].sort, contacts_in_list.map(&:id).sort
  end if Redmine::VERSION.to_s >= '3.3'
  ...
  def test_filter_by_tags_not_equal
    ...
    assert_response :success
    contacts = contacts_in_list
    assert_equal [1, 2, 4, 5, 6].sort, contacts.map(&:id).sort
  end

  def test_filter_by_tags_none
    @request.session[:user_id] = 1
    compatible_request :get, :index, project_id: 1, set_filter: 1, f: ['tags', ''], op: { tags: '!*' }
    assert_response :success
    contacts = contacts_in_list
    assert_equal [5, 6].sort, contacts.map(&:id).sort
    contacts.each { |contact| assert_equal contact.tag_list, [] }
  end
