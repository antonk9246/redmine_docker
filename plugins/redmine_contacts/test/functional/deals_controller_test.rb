...
class DealsControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    @admin_api_key               = User.where(admin: true).first.api_key
    @deal_for_add_product        = Deal.first
    @product_for_add_product     = Product.last if RedmineContacts.products_plugin_installed?
    @description_for_add_product = 'Added product description'
  end
  ...
  def test_get_show_with_notes_with_attachments
    @request.session[:user_id] = 1
    deal = Deal.find(1)
    note = deal.notes.build(subject: 'test', content: 'test', author: User.current)
    note.attachments << Attachment.last.dup
    note.save!

    compatible_request :get, :show, :id => deal.id
    assert_response :success
    assert_select 'h2', 'Deal #1'
    assert_select '.note_data .attachments', 1
  end
  ...
  def test_get_new_with_filtered_contacts
    @request.session[:user_id] = 1
    with_contacts_settings('select_companies_to_deal' => '0') do
      compatible_request :get, :new, :project_id => 1
      assert_response :success
      assert_select 'label[for=deal_contact_id]', 'Contact'
    end
  end

  def test_get_new_with_filtered_companies
    @request.session[:user_id] = 1
    with_contacts_settings('select_companies_to_deal' => '1') do
      compatible_request :get, :new, :project_id => 1
      assert_response :success
      assert_select 'label[for=deal_contact_id]', 'Company'
    end
  end
  ...
  def test_put_update
    @request.session[:user_id] = 1
    with_redminecrm_settings('thousands_delimiter' => ',', 'decimal_separator' => '.') do
      deal = Deal.find(3)
  ...
  def test_add_product_to_deal_unauthorized
    compatible_request :put, :add_product, format:  'json',
                                             id:      @deal_for_add_product.id,
                                             product: { description: @description_for_add_product }
    assert_response :unauthorized
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_for_not_product
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { description: @description_for_add_product }
    assert_response :success
    added_line = @deal_for_add_product.reload.lines.last
    assert_nil added_line.product_id
    assert_equal @description_for_add_product, added_line.description
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_for_not_existent_product_without_description
    not_existent_id = @product_for_add_product.id + 1
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { product_id: not_existent_id }
    assert_response :unprocessable_entity
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_for_not_existent_product
    not_existent_id = @product_for_add_product.id + 1
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { product_id:  not_existent_id,
                                                        description: @description_for_add_product }
    assert_response :success
    added_line = @deal_for_add_product.reload.lines.last
    assert_nil added_line.product_id
    assert_equal @description_for_add_product, added_line.description
  ...
  def test_add_product_to_deal_for_existent_product_without_description
    compatible_request :put, :add_product, format: 'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { product_id: @product_for_add_product.id }
    assert_response :success
    added_line = @deal_for_add_product.reload.lines.last
    assert_equal @product_for_add_product.id, added_line.product_id
    assert_equal @product_for_add_product.description, added_line.description
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_for_existent_product
    product_id = @product_for_add_product.id
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
    ...
    assert_response :success
    added_line = @deal_for_add_product.reload.lines.last
    assert_equal product_id, added_line.product_id
    assert_equal @description_for_add_product, added_line.description
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_with_priceless_line_of_not_existent_product
    old_price = @deal_for_add_product.calculate_price
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { description: @description_for_add_product }
    assert_response :success
    assert_equal old_price, @deal_for_add_product.reload.price
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_with_priceless_line_of_existent_product
    old_price = @deal_for_add_product.calculate_price
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { product_id: @product_for_add_product.id,
                                                        quantity:   2 }
    assert_response :success
    assert_equal old_price + @product_for_add_product.price * 2, @deal_for_add_product.reload.price
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_with_priceful_line_of_not_existent_product
    old_price = @deal_for_add_product.calculate_price
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
                                             product: { description: @description_for_add_product,
                                                        ...
                                                        price:       100 }
    assert_response :success
    assert_equal old_price + 2 * 100, @deal_for_add_product.reload.price
  end if RedmineContacts.products_plugin_installed?

  def test_add_product_to_deal_with_priceful_line_of_existent_product
    old_price = @deal_for_add_product.calculate_price
    compatible_request :put, :add_product, format:  'json',
                                             key:     @admin_api_key,
                                             id:      @deal_for_add_product.id,
