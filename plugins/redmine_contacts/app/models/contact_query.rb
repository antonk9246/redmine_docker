...
class ContactQuery < Query
  ...
  def sql_for_tags_field(field, operator, value)
    compare   = ['=', '*'].include?(operator) ? 'IN' : 'NOT IN' #operator_for('tags').eql?('=')
  ...
  def sql_for_is_company_field(field, operator, value)
    ...
    condition = operator == '=' ? 'IN' : 'NOT IN'
    with_null = [['=', false], ['!', true]].any? { |o, v| operator == o && value.first == v }
    sql = queried_class.send(:sanitize_sql_for_conditions, ["#{Contact.table_name}.is_company #{condition} (?) ", value])
    sql += " OR #{Contact.table_name}.is_company IS NULL" if with_null
    "(" + sql + ")"
  end
