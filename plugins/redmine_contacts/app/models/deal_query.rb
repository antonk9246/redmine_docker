...
class DealQuery < Query
  ...
  def initialize_available_filters
    ...
    add_available_filter('contact_id', type: :contact, name: l(:label_contact))
  ...
  def sql_for_contact_id_field(_field, operator, value)
    case operator
    when '=', '!'
      sw = operator == '!' ? 'NOT' : ''
      "(#{queried_table_name}.contact_id #{sw} IN (#{value.join(',')}))"
    when '*', '!*'
      sw = operator == '!*' ? '' : 'NOT'
      "(#{queried_table_name}.contact_id IS #{sw} NULL)"
    end
  end
