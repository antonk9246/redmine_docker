...
module ContactsHelper
  ...
  def contacts_to_xls(contacts)
    ...
    order_condition = 'LOWER(name)'
    order_condition += ' COLLATE "C"' if ActiveRecord::VERSION::MAJOR < 6 && ActiveRecord::Base.connection.adapter_name.downcase =~ /postgresql/
    custom_fields = ContactCustomField.order(order_condition)
    ...
    contacts.each do |contact|
      ...
      contact.custom_field_values.sort_by{|v| v.custom_field.name.downcase}.each {|custom_value| fields << RedmineContacts::Utils::CsvUtils.csv_custom_value(custom_value) }
      row.replace fields
    end
  ...
  # Use new CustomField to add related_issues to contacts
  def add_contact_issue(issue_id, contact_id)
    contacts_cf = CustomField.where(type: 'IssueCustomField', field_format: 'contact', name: 'Related contacts')
                             .first_or_initialize
    contacts_cf.safe_attributes = { is_for_all: true, multiple: true }
    contacts_cf.tracker_ids = Tracker.pluck(:id)
    contacts_cf.save

    contacts_cf.custom_values.find_or_create_by(customized_type: Issue,
                                                customized_id: issue_id,
                                                value: contact_id)
  end
 
  # generate issue description on add issue render
  def add_issue_description 
    return unless @contact
    description = ""
    description << "*#{l(:field_contact_telegram)}:* #{@contact.telegram} \n" if @contact.telegram.present?
    description << "*#{l(:field_contact_linkedin)}:* #{@contact.linkedin} \n" if @contact.linkedin.present?
    description << "*#{l(:field_contact_salary)}:* #{@contact.salary} \n" if @contact.salary.present?
    description << "*#{l(:field_contact_vacation)}:* #{@contact.vacation} \n" if @contact.vacation.present?
    description << "*#{l(:field_contact_work_with_vpn)}:* #{@contact.work_with_vpn} \n" if @contact.work_with_vpn.present?
    description << "*#{l(:field_contact_english)}:* #{@contact.english} \n" if @contact.english.present?
    description << "*#{l(:field_contact_location)}:* #{@contact.region} \n" if @contact.region.present?
    description << "*#{l(:field_contact_relocation)}:* #{@contact.relocation} \n" if @contact.relocation.present?
    description << "*#{l(:field_contact_readiness_to_start_work)}:* #{@contact.readiness_to_start_work} \n" if @contact.readiness_to_start_work.present?
    description << "*#{l(:field_contact_type_of_employment)}:* #{@contact.type_of_employment} \n" if @contact.type_of_employment.present?
    description << "*#{l(:field_contact_availability_of_equipment)}:* #{@contact.availability_of_equipment} \n" if @contact.availability_of_equipment.present?
    description << "*#{l(:label_add_information)}:* #{@contact.background} \n" if @contact.background.present?
    description
  end
