...
module DealsHelper
  ...
  def deals_to_csv(deals)
    ...
    export = FCSV.generate(:col_sep => l(:general_csv_separator)) do |csv|
      ...
      deals.each do |deal|
        ...
        deal.custom_field_values.sort_by{|v| v.custom_field.name}.each {|custom_value| fields << RedmineContacts::Utils::CsvUtils.csv_custom_value(custom_value) }
  ...
  def add_deal_issue(issue_id, deal_id)
    deals_cf = CustomField.where(type: 'IssueCustomField', field_format: 'deal',
                                 name: 'Related deals').first_or_initialize
    deals_cf.tracker_ids = Tracker.pluck(:id)
    deals_cf.save!

    deals_cf.custom_values.find_or_create_by(customized_type: Issue,
                                             customized_id: issue_id,
                                             value: deal_id)
  end

  def remove_deal_issue(issue_id, deal_id)
    deals_cf = CustomField.where(type: 'IssueCustomField', field_format: 'deal', name: 'Related deals').first

    return unless deals_cf

    d_relation = deals_cf.custom_values.find_by(customized_type: Issue,
                                                customized_id: issue_id,
                                                value: deal_id)
