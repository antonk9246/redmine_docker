...
class ContactsController < ApplicationController
  ...
  respond_to?(:accept_atom_auth) ? accept_atom_auth(:index, :show) : accept_rss_auth(:index, :show)
  ...
  def show
    ...
    @contact_deals = Deal.where(contact_id: @contact.id)
    @contact_deals_count = @contact_deals.count
    respond_to do |format|
  ...
  def send_mails
    ...
    contacts.each do |contact|
      begin
        ...
        ContactsBulkMailer.bulk_mail(contact, params).deliver
