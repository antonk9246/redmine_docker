...
class DealsController < ApplicationController
  ...
  if RedmineContacts.products_plugin_installed?
    before_action :find_deal,    only: [:show, :edit, :update, :destroy, :add_product]
  ...
  include SortHelper
  if RedmineContacts.products_plugin_installed?
    accept_api_auth :index, :show, :create, :update, :destroy, :add_product
  ...
  def update
    ...
    if @deal.save
      ...
      @deal.lines.each(&:save) if RedmineContacts.products_plugin_installed?
  ...
  def add_product
    added_line = @deal.lines.new
    added_line.safe_attributes = product_params
    if added_line.save
      @deal.calculate_price
      @deal.save
      respond_to do |format|
        format.api { render_api_ok }
      end
      ...
      respond_to do |format|
        format.api { render_validation_errors(added_line) }
      end
    end
  end
  ...
  def find_product
    @product = Product.find_by(id: params[:product].try(:[], :product_id))
  end

  def product_params
    product_params = params[:product] || {}
    return product_params.except(:product_id) unless @product

    product_params[:description] ||= @product.description
    product_params[:price]       ||= @product.price
    product_params
  end
end
