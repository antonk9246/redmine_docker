...
END_DESC
    ...
    task :convert_links_to_custom_fields => :environment do
      ...
      deals_cf = CustomField.where(type: 'IssueCustomField', field_format: 'deal', name: 'Related deals')
                            .first_or_initialize
    ...
    task :delete_converted_custom_fields => :environment do
      deals_cf = CustomField.where(type: 'IssueCustomField', field_format: 'deal', name: 'Related deals (converted)').first
      deals_cf&.destroy
      contacts_cf = CustomField.where(type: 'IssueCustomField', field_format: 'contact', name: 'Related contacts (converted)').first
      contacts_cf&.destroy
    end
  end
end
