class CalendarController < ApplicationController
  ...
  def form_issue(issue)
    ...
    ret_var << '</tr>'
    ret_var << '<tr>'
    ret_var << '<td colspan="2">' + '<a href="' + issue.navigation_url[:google] + '" target="_blank">' + 
             ERB::Util.html_escape(issue.formatted_address) + '</a>' + '</td>'
    ret_var << '</tr>'
    # ret_var << '<tr>'
    ...
    # ret_var << '</tr>'
    ret_var << '<tr>'
    ret_var << '<td>' + (translate 'field_contact_data') + '</td>'
    ret_var << '<td>' + issue.description + '</td>' rescue '<td></td>'
    ret_var << '</tr>'
    ret_var << '<tr>'
    ret_var << '<td>' + (translate 'start') + '</td>'
  ...
  def get_events
    ...
    if fuser.blank?
      ...
      watched_issues = Issue.joins(:watchers)
                       .where(watchers: { user_id: User.current.id })
      
      issues = watched_issues.where(['((issues.start_date <= ? AND issues.due_date >= ?) OR (issues.start_date BETWEEN ? AND ?)  OR (issues.due_date BETWEEN ? AND ?))',
                                    fbegin.to_s, fend.to_s, fbegin.to_s, fend.to_s, fbegin.to_s, fend.to_s])
                          .where(issues_condition) rescue []
      
      issues2 = watched_issues.where(['issues.start_date >= ? AND issues.start_date <= ? AND issues.due_date IS NULL',
                                    fbegin.to_s, fend.to_s])
                            .where(issues_condition) rescue []
      
      issues3 = watched_issues.where(['issues.start_date IS NULL AND issues.due_date <= ? AND issues.due_date >= ?',
                                    fend.to_s, fbegin.to_s])
                            .where(issues_condition) rescue []
      
      if Setting.plugin_mega_calendar['display_empty_dates'].to_i == 1
        issues4 = watched_issues.where(['issues.start_date IS NULL AND issues.due_date IS NULL AND (issues.created_on BETWEEN ? AND ?)',
                                      fbegin.to_s, fend.to_s])
                              .where(issues_condition) rescue []
      else
        issues4 = []
