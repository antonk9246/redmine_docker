...
module RedmineInvoices
  module FieldFormat
    class InvoiceFormat < Redmine::FieldFormat::RecordList
      add 'invoice'
      self.customized_class_names = nil
      self.multiple_supported = true

      def edit_tag(view, tag_id, tag_name, custom_value, options = {})
        invoices = Invoice.where(id: custom_value.value).to_a unless custom_value.value.blank?
        view.select2_invoice_tag(tag_name, invoices, options.merge(id: tag_id,
                                                                   class: "invoice_cf #{custom_value.custom_field.multiple ? 'select2_multi_cf' : '' }",
