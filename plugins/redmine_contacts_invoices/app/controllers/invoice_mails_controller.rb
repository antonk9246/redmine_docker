...
class InvoiceMailsController < ApplicationController
  ...
  before_action :find_invoice, :authorize
  ...
  def send_mail
    begin
      ...
      delivered = InvoicesMailer.invoice(User.current, @invoice, params).deliver
      @invoice.update(status_id: Invoice::SENT_INVOICE) if delivered && params[:mark_as_sent]
    rescue Exception => e
